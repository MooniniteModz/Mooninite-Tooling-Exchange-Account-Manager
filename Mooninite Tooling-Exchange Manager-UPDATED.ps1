...
function Set-AdminEmail {
    $Global:TechEmail = Read-Host "Enter your work email address that has the necessary permissions to conduct the compliance search"
...
function Set-EmployeeEmail {
    $Global:EmployeeEmail = Read-Host "Enter the email address of the UPT Employee you are wanting to query or edit"
...
function Prepare-UserMailbox {
    ...
    Write-Host "Disabling WS, Active Sync, MAPI, OWA, IMAP, and POP for $Global:EmployeeEmail........" -ForegroundColor Cyan
    ...
    Write-Host "Increasing the retention window to 30-days for $Global:EmployeeEmail......" -ForegroundColor Cyan
    Set-Mailbox $Global:EmployeeEmail -RetainDeletedItemsFor 0 -ErrorAction SilentlyContinue

    Write-Host "Disabling single-item-recovery for $Global:EmployeeEmail......" -ForegroundColor Cyan
    Set-Mailbox $Global:EmployeeEmail -SingleItemRecoveryEnabled $false -ErrorAction SilentlyContinue

    Write-Host "Disabling Manage Folder Assistant for $Global:EmployeeEmail......" -ForegroundColor Cyan
    Set-Mailbox $Global:EmployeeEmail -ElcProcessingDisabled $true -ErrorAction SilentlyContinue

    Write-Host "Removing all holds on $Global:EmployeeEmail......" -ForegroundColor Cyan
...
function Find-FolderIDs {
    ...
    $folderQueries = @()
    $folderStatistics = Get-MailboxFolderStatistics $Global:EmployeeEmail
    ...
    Write-Host "Looking for Folder ID's, please be patient.........."
...
function Run-ComplianceSearch {
    ...
    Write-Host "Search complete! Configuring the Compliance Search Action!!!!" -ForegroundColor Cyan
    Write-Host ""

    $SearchDetails = Get-ComplianceSearch -Identity $Global:SearchName

    if (($SearchDetails).Status -eq "Completed") {
        Write-Host "Pulling Compliance Search Results..." -ForegroundColor Cyan
        ...
        Write-Host "Search Content Size: $($sizeInGB) GB" -ForegroundColor Red
    }
...
function Run-PurgeOperation {
    ...
    Do {
        ...
        Write-Host ""
        Write-Host "######################### Purge Job Details #######################################################################################################" -ForegroundColor DarkCyan
        Write-Host ""
        Write-Host "Search Name:       | $SearchName" -ForegroundColor Cyan
        Write-Host "Purge Name:        | $PurgeName" -ForegroundColor Cyan
        Write-Host "Purge Errors:      | $PurgeErrors" -ForegroundColor Cyan
        Write-Host ""
        Write-Host "Results:           | $ItemsPurged" -ForegroundColor Cyan
        Write-Host ""
        Write-Host ""
        Write-Host ""
        ...
        Write-host ""
        Write-host ""
        Write-host "Checking folder size to see if additional search purges are needed......." -ForegroundColor Cyan
        ...
        if ($sizeInGB -ge 20) {
            ...
            Get-MailboxFolderStatistics -Identity $Global:EmployeeEmail -FolderScope RecoverableItems | Select-Object FolderPath, ItemsInFolder, FolderSize | Format-List
        }
...
function Purge-RecoverableItems {
    ...
    Write-Host "`nStarting purge operation for $Global:EmployeeEmail..." -ForegroundColor Cyan
